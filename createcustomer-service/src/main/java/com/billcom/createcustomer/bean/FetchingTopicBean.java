...
import com.billcom.createcustomer.exception.GetTopicException;
import com.billcom.createcustomer.model.mongo.TopicDocument;
...
public class FetchingTopicBean {
    ...
    public void exit(Integer code)
    {
        ...
        throw new GetTopicException("Closing MS with Code= " +code);
    }

    private TopicDocument fetchFromMongodb(String msName){
        try (MongoClient db = MongoClients.create(Extern.getMongoUrl()))
        {
            ...
            FindIterable<Document> f = orders.find(Filters.eq("microservice", msName.toLowerCase()));
    ...
    private Optional<TopicDocument> getTopicDocument(){
        return Optional.ofNullable(fetchFromMongodb(Extern.getMicroServiceName()));
    }
    ...
    public void fetchingTopicData() {
        try {
            ...
            TopicDocument res = getTopicDocument( )
                    ...
                    } ).orElseThrow( () -> new GetTopicException( "Closing MS with Code= " + 404 ) );
